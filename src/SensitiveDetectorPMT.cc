...
//Include for getting info from the other SD.
#include "G4SDManager.hh"
#include "SensitiveDetector.hh"


SensitiveDetectorPMT::SensitiveDetectorPMT(G4String name) : G4VSensitiveDetector(name){
  //runnum = 0;
  name = "liquid2_log";
}

SensitiveDetectorPMT::~SensitiveDetectorPMT(){;}
...
void SensitiveDetectorPMT::Initialize(G4HCofThisEvent* HCE){
  runnum = 0;
  TrackID = 0;
  ParentID = 0;
  Counter = 0;
...
G4bool SensitiveDetectorPMT::ProcessHits(G4Step* theStep, G4TouchableHistory*){
  ...
  //5. Only an some other particle entered the vial and deposited energy.
  ...
  //G4cout << "Particle Stepping in PMT Vial!" << G4endl;
...
void SensitiveDetectorPMT::EndOfEvent(G4HCofThisEvent*)
{
  //only need to do anything if there were some interactions.
    ...
    if(myRunAction){
      ...
      //G4cout << "Calling RunAction->TallyEdepPMT" << G4endl;
      //SensitiveDetector* pSD = new
      //G4SDManager* SDman = G4SDManager::GetSDMpointer();
      //SensitiveDetector* pSD =
      //(SensitiveDetector*)SDman->FindSensitiveDetector("liquid_log");

      //G4double EdepFr = pSD->GetEdep();

      //G4cout << "Edep in Front Detector = " << EdepFr 
      //     << ". Edep in PMT Detector = " << EdepThisEventUnquenched
      //     << G4endl;

      //if((EdepFr!=0)||(EdepThisEventUnquenched!=0)){
	//log the event with RunAction
	//G4int EvtTypeFr = pSD->GetEvtType();
	//G4double KinEnFr = pSD->GetKinEnIn();
	//G4double TrLenInFr = pSD->GetTrackLenInVol();
	//G4double TrLenPriorFr = pSD->GetTrackLenPrior();
	//G4double VxFr = pSD->GetVertexX();
	//G4double VzFr = pSD->GetVertexZ();
	//G4double VrFr = pSD->GetVertexR();

	//myRunAction->TallyEvtData(EvtTypeFr, KinEnFr, EdepFr,
	//			  TrLenPriorFr, TrLenInFr, VxFr, VzFr, VrFr,
	//			  EvtType, KinEnIn, EdepThisEventUnquenched,
	//			  TrackLenPrior, TrackLenInVol,
	//			  VertexX, VertexZ, VertexR);

      //}

      if(EdepThisEventUnquenched!=0){
	myRunAction->TallyEdepPMT(EdepThisEventUnquenched);
      }
    }
}


G4double SensitiveDetectorPMT::ApplyBirksQuench(G4double En){
...
G4double SensitiveDetectorPMT::EvaluateBirks(G4double En){
  if(En!=0){
    G4bool Interp = false;
    G4double EnOrig;
    if(En<0.1*keV){
