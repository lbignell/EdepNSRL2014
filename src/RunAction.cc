...
void RunAction::BeginOfRunAction(const G4Run* aRun){
  ...
  EdepPMT = new TH1D("Edep with no Quench, PMT vial","Energy Deposition in PMT Scintillant per Event, no quench correction",210000,0.,210000.);
...
void RunAction::TallyEdepPMT(const G4double thisEdep){
  
  //G4cout << "Inside TallyEdepPMT, Edep = " << thisEdep << G4endl;

  G4double  thisEdepkeV = thisEdep*1000;

  //Fill ROOT histogram
  EdepPMT->Fill(thisEdepkeV);
...
void RunAction::EndOfRunAction(const G4Run* aRun){
  ...
  Beginning = "EdepNoQuenchPMT";
  //SpectName = Beginning+RunNum;
  EdepPMT->SetName(Beginning);
  EdepPMT->Scale(1/NumberOfEvents);
  EdepPMT->Write();
