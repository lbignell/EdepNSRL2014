...
SensitiveDetectorRear::SensitiveDetectorRear(G4String name) : G4VSensitiveDetector(name){
  //runnum = 0;
  name = "liquid2_log";
}

SensitiveDetectorRear::~SensitiveDetectorRear(){;}
...
void SensitiveDetectorRear::Initialize(G4HCofThisEvent* HCE){
  runnum = 0;
  TrackID = 0;
  ParentID = 0;
  Counter = 0;
...
G4bool SensitiveDetectorRear::ProcessHits(G4Step* theStep, G4TouchableHistory*){
...
void SensitiveDetectorRear::EndOfEvent(G4HCofThisEvent*)
{
  //only need to do anything if there were some interactions.
	...
	(SensitiveDetector*)SDman->FindSensitiveDetector("liquid_log");
      ...
      //     << ". Edep in Rear Detector = " << EdepThisEventUnquenched
      //     << G4endl;
	...
	myRunAction->TallyEdepRear(EdepThisEventUnquenched);
      //myRunAction->TallyEvtDataRear(EvtType, KinEnIn, EdepThisEventUnquenched,
      //			    TrackLenPrior, TrackLenInVol);
...
G4double SensitiveDetectorRear::ApplyBirksQuench(G4double En){
...
G4double SensitiveDetectorRear::EvaluateBirks(G4double En){
  if(En!=0){
    G4bool Interp = false;
    G4double EnOrig;
    if(En<0.1*keV){
